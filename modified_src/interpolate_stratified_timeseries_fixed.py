...
# Define correct feature and target columns (exclude 'sex' and 'test_time')
feature_columns = [
    "age", "Jitter(%)", "Jitter(Abs)", "Shimmer", "Shimmer(dB)",
...
for subject_id, subject_df in df.groupby("subject#"):
    ...
    for i in range(len(feats) - 1):
        ...
        # Append sex as constant feature (non-interpolated)
        mid_feat = np.append(mid_feat, sex)

        # Append delta_time as a feature
        mid_feat = np.append(mid_feat, delta_time)
